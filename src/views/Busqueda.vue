<template>
    <div class="h-100 w-100 d-flex flex-column align-items-center gap-2">
        <form
            class="d-flex w-75"
            role="search"
            v-on:submit.prevent="cargarResultados()"
        >
            <input
                class="form-control me-2 border border-1 border-secondary"
                type="search"
                id="busquedaECBE"
                placeholder="Buscar ECBE o preguntar!"
                aria-label="Search"
                required
            />
        ...
        <div
            class="w-100 rounded-top-3 border-bottom border-dark border-1"
            style="background-color: rgb(169, 203, 227)"
        >
            <p v-if="!esRespuesta" class="text-center p-0 m-1 opacity-25">
                Resultados
            </p>
            <p v-else class="text-center p-0 m-1">
                {{ dataRespuesta.content }}
            </p>
        </div>
        <div v-if="!esRespuesta" class="card mb-3" style="max-width: 540px">
            <div class="row g-0">
                <div class="col-4">
                    <img
                        src="@/assets/grey.png"
                        class="img-thumbnail rounded-start w-100 h-100"
                        alt="Imagen"
                    />
                </div>
                <div class="col-8">
...
export default {
    methods: {
        async cargarResultados() {
            try {
                await funciones
                    .buscarECBE(document.getElementById("busquedaECBE").value)
                    .then((res) => {
